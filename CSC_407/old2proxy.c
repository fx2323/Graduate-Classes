...
int rio_writen_w(int fd, void *usrbuf, size_t n){
  if(Signal(SIGPIPE,(void *)Rio_writen)==SIG_ERR){return -1;}
  if(Signal(SIGTERM,(void *)Rio_writen)==SIG_ERR){return -1;}
 ...
 * pthreadExit
 */
void pthreadExit(int errorCode, int thread, int clientfd, int serverfd){
  switch (errorCode){
  case 0:{break;}
  case -1:{printf("Thread %d: invailed request\n",thread); break;}
  case -2:{printf("Thread %d: fail to connect to server\n",thread);break;}
  case -3:{printf("Thread %d: fail to send request\n",thread);break;}
  case -4:{printf("Thread %d: fail to recieve data\n",thread);break;}
  case -5:{printf("Thread %d: fail to recieve data\n",thread);break;}
  }
...
void* doit(void * data){
  ...
  int   clientfd=(*info).clientfd, serverfd=0,thread=(*info).thread;
  rio_t rioWithClient,rioWithServer;
  struct sockaddr_in *sockaddr=&(*info).sockaddr;
  int  n, size=0,portValue=0,dataNeeded,transferSize=MAXLINE;
  int  *port=&portValue;
  ...
  //builds the request from client if fails closes
  struct requestInfo getRequest=request(rioWithClient,port);
  if(*getRequest.request==NULL){pthreadExit(-1,thread,clientfd,serverfd);}
  ...
  //connecting to server if fails closes
  serverfd=Open_clientfd((char *)getRequest.hostname,*getRequest.port);
  if(serverfd<0){pthreadExit(-2,thread,clientfd,serverfd);}
  rio_readinitb(&rioWithServer,serverfd);

  //send request if fails closes
  if(rio_writen_w(serverfd,getRequest.request,strlen((const char *)getRequest.request))<0){
    pthreadExit(-3,thread,clientfd,serverfd);}

  //get headers from server to find the length and also transfer to client
  strcpy((char *)buf,"");
  while(strcmp((const char *)buf,"\r\n")!=0){
    if((n=rio_readlineb_w(&rioWithServer,buf,MAXLINE))==0){pthreadExit(-3,thread,clientfd,serverfd);}
    size+=n;
    if(rio_writen_w(clientfd,buf,n)<0){pthreadExit(-4,thread,clientfd,serverfd);}
    sscanf(buf, "Content-Length: %d",&dataNeeded);
  }
  printf("size: %d\ndataNeeded: %d\n",size,dataNeeded);
  //receive request from server then send to client
  while((n=rio_readnb_w(&rioWithServer,buf,MIN(transferSize,dataNeeded)))>0){
    size +=n;dataNeeded-=n;
    if(rio_writen_w(clientfd,buf,n)<0){pthreadExit(-4,thread,clientfd,serverfd);}
    printf("Thread %d: Forwarded %d bytes from end server to client\n",thread,n);
  }
  if(n<0){ pthreadExit(-5,thread,clientfd,serverfd);}
  ...
  //closes with success
  pthreadExit(0,thread,clientfd,serverfd);
  return NULL;
}
