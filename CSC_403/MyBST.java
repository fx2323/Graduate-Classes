...
//        else                 return max(x.right); 
    ...
    private Node floor(Node x, Key key) {
    	...
    	while(true){
    		...
    		if (cmp == 0) return x;
    		if (cmp <  0) x=x.left;
    		if (cmp > 0 && x.right!=null) x=x.right;
    		else return x;
    	}
...
//        else            return x; 
    ...
    private int rank(Key key, Node x) {
    	int sum=0;
    	while(true){
    		if (x == null) return 0+sum; 
    		int cmp = key.compareTo(x.key);
    		if      (cmp < 0) x=x.left;
    		else if (cmp > 0){ sum =+( 1 + size(x.left)); x=x.right;}
    		else              return size(x.left)+sum; 
    	}
  ...
  /*************************************************************************
    ...
    *****************************************************************************/
    public static void main(String[] args) { 
        ...
        StdOut.println("Floor of input B: "+st.floor("B"));
        StdOut.println("Ceiling of input M: "+st.ceiling("M"));
        StdOut.println("Select(4): "+st.select(4));
        StdOut.println("rank('F'): "+st.rank("F"));
    }
}
